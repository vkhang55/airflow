...
from airflow.contrib.operators.aws_dms_operator import AWSDMSOperator
...
class TestAWSDMSOperator(unittest.TestCase):

    @mock.patch('airflow.contrib.operators.aws_dms_operator.AwsHook')
    def setUp(self, aws_hook_mock):
        configuration.load_test_config()

        self.aws_hook_mock = aws_hook_mock
        self.dms = AWSDMSOperator(
            replication_task_arn='arn_id',
    ...
    def test_init(self):
        self.assertEqual(self.dms.replication_task_arn, 'arn_id')
        self.assertEqual(self.dms.start_replication_task_type, 'start-replication')
        self.assertEqual(self.dms.max_retries, 4200)
        self.assertEqual(self.dms.aws_conn_id, None)
        self.assertEqual(self.dms.region_name, 'us-west-2')
        self.assertEqual(self.dms.hook, self.aws_hook_mock.return_value)
        self.aws_hook_mock.assert_called_once_with(aws_conn_id=None)

        # TODO: Resume

    @mock.patch.object(AWSDMSOperator, '_wait_for_task_ended')
    @mock.patch.object(AWSDMSOperator, '_check_success_task')
